...
def clean_up_sentence(sentence: str, lemmatizer) -> list:
...
def bag_of_words(sentence: str, lemmatizer, words) -> np.array:
    ...
    sent_words = clean_up_sentence(sentence, lemmatizer)
    bag = [0] * len(words)
    for w in sent_words:
...
def setup():
    ...
    return (intents, words, classes, model, lemmatizer)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if message == "ajd bok":
            break   
        ints = predict_class(message, model, lemmatizer, classes, words)
